...
<form name="order_detail_form" action="" enctype="multipart/form-data" method="post" next-url="">
    <div class="container-fluid m_top_30 nycol_list  sys_mod">
        <div id="myTabContent" class="tab-content">
            <div class="tab-pane fade in active" id="home">
                <table class="layui-table" lay-size="sm">
					...
					<tr style="background-color:#f6f6f6; ">
                        ...
                        <td width="15%"><#if orders.payMethod?? && orders.payMethod="ALIPAY">支付宝<#else>积分</#if></td>
                        <td width="20%">${(orders.createTime?string("yyyy-MM-dd HH:mm:ss"))!}</td>
						<td width="20%">${orders.recommendUser!}</td>
                    </tr>
                    <tr>
                        <td colspan="5">
                            <table class="layui-table" lay-size="sm">
            ...
            <#if orderExchange.toExpressNo??>
                <table class="layui-table" lay-size="sm">
                    <caption>发货物流信息</caption>
                    <tr style="font-weight:bold;">
                        <td>物流公司</td>
                        <td>运单号</td>
                        <td>配送费用</td>
                    ...
                    <tr>
                        <td>${(orderExchange.toExpressCompName)!}</td>
                        <td>${(orderExchange.toExpressNo)!}</td>
                        <td>￥${(orders.deliveryFee)!}.00</td>
                        <td>
                            <#if orders.isInsured?? && ( orders.isInsured == 1 )>是<#else>否</#if>
                        </td>
...
<form id="makesure">
  <div class="modal-content">
    ...
    <div class="modal-footer">
      ...
      <button tyclass="layui-btn layui-btn-primary"btn-default" data-dismiss="modal1">关闭</button>
...
<form id="orderback">
  <div class="modal-content">
    ...
    class="layui-btn layui-btn-primary" class="btn btn-default" data-dismiss="modal2">关闭</button>
...
<form id="orderrefund">
  <div class="modal-content">
      ...
      <table class="table table-hover">
		<tbody id="jiageTB">
		   ...
		   <tr>
            <td>退款金额:</td>
			<td>
				<input type="text" data-rule="required;range(0~);" name="amount" id="amount" value='${(orders.totalPrice?string("0.00"))!}'/>
			</td>
		   </tr>
		   <tr>
            <td>退款渠道:</td>
    ...
    <div class="modal-footer">
	  <button type="button" class="layui-btn layui-btn-normal" data-save="modal3">确定</class="layui-btn layui-btn-primary"ype="button" class="btn btn-default" data-dismiss="modal3">关闭</button>
