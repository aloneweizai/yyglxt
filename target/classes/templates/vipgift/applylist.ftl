                    ...
                    <#if applylists?? && ( applylists?size gt 0 )> 
					  <#list applylists as apply>
					   <tr>
					     ...
					     <td>${(apply.applyId)!}</td>
						 <td>${(apply.name)!}</td>
                           <td><a class="ljc_00bcd4" data-type="opendialog" data-val="4" href="javascript:;" data-url="${ctx}/consumerManager/consumer/info.php?id=${apply.userId}">${(apply.username)!}</a></td>
						 <td>${(apply.phone)!}</td>
