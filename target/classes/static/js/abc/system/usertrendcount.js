require(["../../config"], function () {
    require(["jquery.full","jqplot.full","abc.common"], function ($) {
      $(function () {
		  ...
		  $.ajax({
			  type: "GET",
			  url: ctx+"/usertrendcount/registerusercount/listJson.php?type="+type+"&&timed="+timed+"&&timeb="+timeb,
			  async: true,
			  contentType: "application/json",
			  dataType: "JSON",
			  success: function (data) {
				  $("#registeruser").html("");
				  var html = "<tr><th width='30'>序号</th><th>日期</th><th>注册用户数（人）</th>";
				  $.each(data.userTrendCountRs, function (i, item) {
					  html+= "<tr><td width='4%'>"+(i+1)+"</td>";
					  html+= "<td>"+item.days +"</td>";
					  html+= "<td><a class='opendialog ljc_00bcd4' href='javascript:void (0)' data-url='/usertrendcount/registerusercount/userlook.php?days="+item.days+"'>"+item.count +"</a></td>";
					  html+= '</tr>';
				  });
