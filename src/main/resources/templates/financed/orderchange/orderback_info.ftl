...
<body>
<form name="orderback" id="orderback" action="" enctype="multipart/form-data" method="post" next-url="">
        ...
        <div id="myTabContent" class="tab-content">
                ...
                <table class="layui-table" lay-size="sm">
                            ...
                            <#if orders.orderProductBOList?? && ( orders.orderProductBOList?size gt 0 )>
                                <#list orders.orderProductBOList as orderProduct>
                                    <tr style="background-color:#f6f6f6; ">
                                        ...
                                        <#else>
                                            <td><#if orderProduct.dealPrice??>
                                                ￥${orderProduct.dealPrice?string("0.00")!}</#if></td>
                                        </#if>
                    ...
                    <tr style="background-color:#f6f6f6; ">
                        ...
                        <#if orderExchange.type=="1">
                            <div class="btn btn-info btn-xs ">换货</div><#else>
                    ...
                    <tr style="font-weight:bold;">
                        <td colspan="4">用户申请备注</td>
                    </tr>
                    <tr>
                        <td colspan="4" style="background-color:#f6f6f6;height:80px;">${orderExchange.userRemark!}</td>
                    </tr>
                </table>
            <#if orderExchange.type=="1">
                <table class="layui-table" lay-size="sm">
            ...
            <#if orders.isShipping?? && ( orders.isShipping == 1 )&&orderExchange.type=="1">
                ...
                <table class="layui-table" lay-size="sm">
                    ...
                    <#if (orders.invoiceBO?? && orders.invoiceBO.property=="1")>
                    <tr>
                        <td>收货快递公司</td>
                        <td>
                            <input type="hidden" name="id" id="goodsType" value="${orderExchange.goodsType!}"/>
                            <#if orders.invoiceBO??>
                                <input type="hidden" name="id" id="property" value="${orders.invoiceBO.property!}"/>
                            </#if>
                            <input type="text" name="expressComp" id="expressComp"
                                   style="width:250px;height:30px;"><label style='color:red;'>*</label><label id="tips"
                    ...
                    <tr>
                        <td>收货快递单号</td>
                        <td>
                            <input type="text" name="expressNo" id="expressNo" style="width:250px;height:30px;"><label
                                style='color:red;'>*</label><label id="tips1" style='color:red;font-size: x-small;'
                                                                   hidden>不能为空</label>
				...
				<textarea id="refuseRson" placeholder="实物退单请确认已收到货物;如已开纸质发票请确认是否已收到发票" name="adminConfirmRemark" rows="6" cols="50" ></textarea>
                            <br/>
...
<form id="orderback">
  <div class="modal-content">
      ...
      <table class="table table-hover">
		<tbody id="jiageTB">
		   ...
		   <tr>
			...
			<td>
				<input type="hidden" name="id" id="orderExchangeIdback" value="${orderExchange.id!}"/>
				<textarea id="refuseRson" placeholder="实物退单请确认已收到货物;如已开纸质发票请确认是否已收到发票"  name="adminConfirmRemark" rows="6" cols="50" data-rule="required;" >
                </textarea><label style='color:red;'>*</label>
                <br/>
                &lt;#&ndash;<span style="font-size:12px; color: #999;">温馨提示:实物退单请确认已收到货物;如已开纸质发票请确认是否已收到发票</span>&ndash;&gt;
			</td>
		   </tr>
		</tbody>
      </table>
    ...
    <div class="modal-footer">
	  <button type="button" class="layui-btn layui-btn-normal" data-save="modal2">确定</button>
      <button type="button" class="layui-btn layui-btn-primary" data-dismiss="modal2">关闭</button>
