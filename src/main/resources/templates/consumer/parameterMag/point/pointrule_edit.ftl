  ...
  <div class="container-fluid m_top_30 nycon_list sys_mod_add">
    ...
    <table class="table  table-hover">
      <tr style="background:#f5f5f5">
        <td height="30">&nbsp;当前位置：&nbsp;
          <a href="${ctx}/consumerManager/pointrule/list.php">
            <u>积分规则设置</u>
          </a> &gt;&gt; 添加/编辑积分规则</td>
      </tr>
    </table>
	-->
    <form name="consumer_edit" action="${ctx}/consumerManager/pointrule/save.php" next-url="${ctx}/consumerManager/pointrule/list.php" method="post" class="layui-form">
      <table class="layui-table" lay-size="sm">
		<#if pointRule??>
		<input type="hidden" name="id" value="${pointRule.id!}">
        <tr>
          <td width="90">规则名称</td>
          <td colspan="3"><input type="text" name="name" data-rule="required;length[2~16]" value="${pointRule.name!}"  class="layui-input" style="width:330px;" class="layui-input" ><label style='color:red;'>*</label></td>
        </tr>
        <tr>
          <td>规则代码</td>
          <td colspan="3">
			<input type="text" name="codeHead" value="P" readonly  style="width:35px;"  class="layui-input">-
			<input type="text" name="code" data-rule="required;length[2~10]" value="${pointRule.code?substring(2)}"  style="width:285px;"  class="layui-input"><label style='color:red;'>*</label>
          </td>
        </tr>
		<tr>
          <td>积分</td>
          <td colspan="3"><input type="text" name="points" data-rule="required;allint;" value="${pointRule.points?c}" style=" width:330px;" class="layui-input"><label style='color:red;'>*</label></td>
        </tr>
		<tr>
          <td>计算周期</td>
          <td colspan="3">
		...
		<tr>
          <td>次数</td>
          <td colspan="3">
			<input data-rule='required;integer(+0);length(1~5);' value="${pointRule.degree!}" name='degree'  type="text" style="width: 100px;"  class="layui-input"/>
			<label style='color:red;' >*</label></td>
        </tr>
		<tr>
          <td>描述</td>
          <td colspan="3"><textarea name="description" data-rule="length[~500]" rows="3" cols="60" class="layui-textarea">${pointRule.description!}</textarea></td>
        </tr>
        <tr>
          <td>状态</td>
        ...
        <tr>
          ...
          <td colspan="3">
			<input type="text" name="codeHead" value="P" readonly  style="width:35px;"  class="layui-input">-
			<input type="text" name="code" data-rule="required;length[2~10]" style=" width:285px;" class="layui-input"><label style='color:red;'>*</label></td>
        </tr>
		<tr>
          <td>积分</td>
          <td colspan="3"><input type="text" name="points" data-rule="required;allint;" style=" width:330px;" class="layui-input"><label style='color:red;'>*</label></td>
        </tr>
		<tr>
          <td>计算周期</td>
          <td colspan="3">
