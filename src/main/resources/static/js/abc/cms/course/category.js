...
define(['jquery-3.1.0','ztree', 'ztree.excheck','ztree.exedit','blockUI','layui'],function(){
    ...
    function initUrl(listUrl,modifyUrl,deleteUrl,saveUrl){
        //ajaxList_url = ctx + "/cms/course/cate/ajaxList.php";
        //modify_url = ctx +"/cms/course/cate/save.php";
        //delete_url = ctx +"/cms/course/cate/delete/{id}.php";
        //save_url = ctx +"/cms/course/cate/save.php";
        ...
        $.ajax({
            ...
            success : function(data){
                ...
                $.each(data,function(key, val){
                    ...
                    zNodes.push(obj);
                    var li = '<li class="category_li" data-id="'+val.id+'" data-code="'+val.classifyId+'" data-pCode="'+val.parentId+'" data-name="'+val.classifyName+'" data-open="'+obj.open+'"></li>';
                    ...
                    success:function(){
                        ...
                        $(".js_save_cate").on('click',function(){
                            ...
                            $.ajax({
                                type:'POST',
                                url: save_url,
                                data:JSON.stringify({"classifyId":node.id,"classifyName":name,parentId:parentCode,"priority":sort}),
                                async: false,
                                dataType:'json',
                                contentType: "application/json",
                                success : function(result){
    ...
    $("#removeCateBtn").on('click',function(){
        ...
        if(nodes.length == 1){
            ...
            layer.confirm("是否确认删除分类？", {title:'操作提示',btn: ['确认','取消'], icon: 3, closeBtn: 0},function() {
                $.ajax({
                    type:'DELETE',
                    url: delete_url.replace("{id}", nodes[0].idx),
                    success : function(result){
                        if (result.code == "2000") {
                    ...
                    success:function(){
                        ...
                        $(".js_save_cate").on('click',function(){
                            ...
                            $.ajax({
                                type:'POST',
                                url: save_url,
                                data:JSON.stringify({"parentId":parentCode,"classifyName":name,"priority":sort,"isDisplay":1}),
                                async: false,
                                dataType:'json',
                                contentType: "application/json",
                                success : function(result){
