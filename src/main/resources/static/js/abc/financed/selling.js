    ...
    require(["jquery.full","select2.full"], function ($) {
        ...
        $("#prodcutslect").change(function(){
            ...
            if("HYCZ" == _val){
                ...
                $.ajax({
                    type: "GET",
                    url: ctx+"/financed/queryCourse.php?vipLevel="+vipLevel+"&username="+$('#cusername').text(),
                    async: false,
                    contentType: "application/json",
                    dataType: "JSON",
                    success: function (data) {
                    ...
                    success: function (data) {
                        if (data) {
                            $.each(data, function (i, item) {
                                ...
                                gift.append("<div class='checkbox'><input type='checkbox' checked='checked' id='zengsong' disabled/><label for='zengsong'>"+item.sendPoints+"积分</label></div>");
                            });
                        }
                    }
                });
                            ...
                            $.each(uvipPriceBoList, function (i, item) {
                                if (vipLevel == item.vipGrade) {
                                    ...
                                    for (var j = 0; j < curriculumGiftBoList.length; j++) {
                                        ...
                                        if ("COUPON" == operType) {
                                            var _div = $('<div class="checkbox"></div>');
                                            var _coupon = $("<input name='gift' id='coupon1' type='checkbox' operValue='"+curriculumGiftBoList[j].operValue+"' operType='"+curriculumGiftBoList[j].operType+"' giftId='"+curriculumGiftBoList[j].giftId+"' operid='"+curriculumGiftBoList[j].id+"'operSymbol='"+curriculumGiftBoList[j].operSymbol+"'/>").click(function(){
                                            ...
                                            _div.append(_coupon);
                                            _div.append("<label for='coupon1'>赠送优惠券："+ getCoupon(curriculumGiftBoList[j].operValue)+"</label>")
                                            gift.append(_div);
                                           // gift.append("<input type='checkbox' style='width:25px;height:25px' operid='"+curriculumGiftBoList[j].id+"'operSymbol='"+curriculumGiftBoList[j].operSymbol+"'/>赠送优惠券：" + curriculumGiftBoList[j].operValue);
                                            ...
                                            var _div = $('<div class="checkbox"></div>')
                                            var _amount = $("<input  name='gift' type='checkbox' id='amount1' operValue='"+curriculumGiftBoList[j].operValue+"' operType='"+curriculumGiftBoList[j].operType+"' giftId='"+curriculumGiftBoList[j].giftId+"' operid='"+curriculumGiftBoList[j].id+"' operSymbol='"+curriculumGiftBoList[j].operSymbol+"'/>").click(function(){
                                                var operSymbol = $(this).attr("operSymbol");
                                                var operid = $(this).attr("operid");
                                                var bools = $(this).is(":checked");
                                                if(bools){
                                            ...
                                            _div.append(_amount);
                                            _div.append("<label for='amount1'>"+curriculumGiftBoList[j].operValue+"元会员礼包&nbsp;</label>");
                                            gift.append(_div);
                                            ...
                                            var _div = $('<div class="checkbox"></div>')
                                            var _point = $("<input  name='gift' type='checkbox' id='point1' operValue='"+curriculumGiftBoList[j].operValue+"' operType='"+curriculumGiftBoList[j].operType+"' giftId='"+curriculumGiftBoList[j].giftId+"' operid='"+curriculumGiftBoList[j].id+"' operSymbol='"+curriculumGiftBoList[j].operSymbol+"'/>").click(function(){
                                                var operSymbol = $(this).attr("operSymbol");
                                                var operid = $(this).attr("operid");
                                                var bools = $(this).is(":checked");
                                                if(bools){
                                            ...
                                            _div.append(_point);
                                            _div.append("<label for='point1'>"+curriculumGiftBoList[j].operValue+"积分&nbsp;</label>");
                                            gift.append(_div);
                                           // gift.append("<input type='checkbox' style='width:25px;height:25px' operid='"+curriculumGiftBoList[j].id+"' operSymbol='"+curriculumGiftBoList[j].operSymbol+"'/>" + curriculumGiftBoList[j].operValue + "积分");
                                            ...
                                            var _div = $('<div class="checkbox"></div>')
                                            var _vip = $("<input  name='gift' type='checkbox' id='vip1' operValue='"+curriculumGiftBoList[j].operValue+"' operType='"+curriculumGiftBoList[j].operType+"' giftId='"+curriculumGiftBoList[j].giftId+"' operid='"+curriculumGiftBoList[j].id+"' operSymbol='"+curriculumGiftBoList[j].operSymbol+"'/>").click(function(){
                                                var operSymbol = $(this).attr("operSymbol");
                                                var operid = $(this).attr("operid");
                                                var bools = $(this).is(":checked");
                                                if(bools){
                                            ...
                                            _div.append(_vip);
                                            _div.append("<label for='vip1'>"+getVipLevel(curriculumGiftBoList[j].operValue)+"</label>");
                                            gift.append(_div);
        ...
        $("#editbtn").click(function () {
            ...
            if (name == null || name == "") {
                abc.layerAlert(false, "请选择下单的产品规格！");
                return;
            }
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if(data.code == '2000'){
                            ...
                            $("#yyy").hide();
                            $("#myModal").hide();
                            layer.msg(data.message, {offset: abc.winscrollTop(200), shade: 0.3, icon: 1, time: 1000});
                            location.reload();
                        }
                    }
                });
        ...
        $("[name='wanchengzhifu']").click(function () {
            ...
            if("ali"==_val) {
                $.ajax({
                    type: "GET",
                    url: ctx + "/financed/ali/tradelog.php?out_trade_no=" + tradeNo,
                    async: true,
                    contentType: "application/json",
                    dataType: "JSON",
                    success: function (data) {
