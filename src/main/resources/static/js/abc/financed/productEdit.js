	...
	require([  "jquery.full","wangEditor","ztree","abc.ajaxfileupload","bootstrap"], function($,Editor) {
		...
		$(function() {
	        ...
	        $(document).ready(function(){
	        	...
	        	$.ajax({
	                type: "GET",
	                url: ctx+"/financed/goodProduct.php?goodsId="+_goodId,
	                async: false,
	                ...
	                success: function (data) {
	                    if(data.length>0){
	                      for(var i=0;i<data.length;i++){
	                    	var _data=data[i];
	                    	var _p={'stock':0,'weight':0};
	                    	_p["id"]=_data.id;
	                    	_p["marketPrice"]=_data.marketPrice;
	                    	_p["sellingPrice"]=_data.sellingPrice;
	                    	_p["costPrice"]=_data.costPrice;
	                    	if(_data.dictList.length>0){
	                    		var _dictList=_data.dictList;
	                    		for(var j=0;j<_dictList.length;j++){
	                    		   var _dict=_dictList[j];
	                    		   if(typeof(selectBigRules[_dict.dictId+"!"+_dict.dictName])=="undefined"){
	              	        		  selectBigRules[_dict.dictId+"!"+_dict.dictName]=new Array();
	              	        		  selectBigRules[_dict.dictId+"!"+_dict.dictName].push(_dict.id);
	              	        	   }else if(typeof(selectBigRules[_dict.dictId+"!"+_dict.dictName])!="undefined"){
	              	        		  selectBigRules[_dict.dictId+"!"+_dict.dictName].push(_dict.id);
	              	        	   }
	                    		   _p[_dict.dictId]=_dict.id;
	            	        	   _p[_dict.dictId+"_tx"]=_dict.fieldKey;
	                    		}
	                    	}
	                    	var uvipPriceList=new Array();
	                    	$.ajax({
	                            type: "GET",
	                            url: ctx+"/financed/productPrice.php?productId="+_data.id,
	                            async: false,
	                            contentType: "application/json",
	                            dataType: "JSON",
	                            success: function (data) {
	                            	for(var i=0;i<data.length;i++){
	                            		var _datas=data[i];
	                            		var uvipPrice={};
	                            		uvipPrice["vipLevel"]=_datas.vipLevel;
	                            		uvipPrice["discount"]=_datas.discount;
	                            		uvipPrice["tradePrice"]=_datas.tradePrice;
	                            		uvipPrice["giftPoints"]=_datas.giftPoints;
	                            		uvipPriceList.push(uvipPrice);
	                            	}
	                            }
	                        });
	                    	_p["uvipPriceList"]=uvipPriceList;
	        	...
	        	if(look=='yes'){
	        		$("input").attr('readonly','readonly').css('cursor','not-allowed');
	        		$("textarea").attr('readonly','readonly').css('cursor','not-allowed');
	        		$("#consumer_submit").css('display','none');
	        		$("#guigetr").css('display','none');
	        		editor.$textElem.attr('contenteditable', false);
	        		$("[type='radio']").attr('disabled','disabled');
	        		$("select").attr('disabled','disabled');
	        		$("#uploadFiletd").css("visibility","hidden");
	        		$("input[name='categoryName']").unbind("click");
	        		$("[data-save='modal']").css('display','none');
	        	}
	        	if($("#goodsType").val()=='4'){
            ...
            $.fn.zTree.init($("#treeDemo"), setting, zNodes);
	          ...
	          function showTable(){
		    	  ...
		    	  for(var i=0;i<proudcts.length;i++){
		    		  ...
		    		  _tr.append('<td trindex="'+i+'" width="250"><input type="text" data-di id="sellingPrice" style="width:90px;" value="'+_pt["sellingPrice"]+'">&nbsp;&nbsp;</td><td><a class="btn btn-primary btn-sm" data-alert="alertDiv"><i class="glyphicon glyphicon-plus">会员组价格</i></td>');
			...
			function chek(val){
	          ...
	          function vipPrice(object){
				  ...
				  //selectXsjg*parseFloat((_jgs[i])['discount'])/100
	        	  ...
	        	  if(_jgs.length>0){
	        		  ...
	        		  for(var i=0;i<_jgs.length;i++){
	        			  ...
	        			  _trs.find("tr[data-vip='"+_vip+"']").find("#discount").val((_jgs[i])['discount']).css('border-color','#ccc').blur();
	    	  ...
	    	  var $validatorWsysVoFrom = $("form1").validator({
	    		  timely: 1,
	    		  focusCleanup:true,
	          ...
	          $("#consumer_submit").click(function(){
	        	  ...
	        	  if($validatorWsysVoFrom.isValid()){ 
	                  ...
	                  for(var k=0;k<proudcts.length;k++){
		    			  ...
		    			  for(var _p=0;_p<_pp.uvipPriceList.length;_p++){
		    				  var _d=_pp.uvipPriceList[_p];
		    				  _d['tradePrice']=(_d.discount*_pp.sellingPrice)/100;
		    			  }
