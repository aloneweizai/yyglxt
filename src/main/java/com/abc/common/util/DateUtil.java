...
public class DateUtil {
    ...
    public static String addYears(java.util.Date paramDate,
                                           int paramInt) {
        Calendar localCalendar = Calendar.getInstance();
        localCalendar.setTime(paramDate);
...
//        }
    ...
    /**
     * 判断日期1是否大于日期2，精确到日期进行比较，考虑时间
     * @param d1 日期1
     * @param d2 日期2
     * @return  大于true 小于等于false
     */
    public static boolean compareTimeBig(Date d1,Date d2){
        if(d1==null ||d2==null)
            return false;
        return d1.compareTo(d2)==1;
    }
 ...
 /*
    ...
    */
    ...
    public static List<Date> dateToLastWeek(Date mdate) {
        ...
        Date fdate;
        List<Date> list = new ArrayList<Date>();
        Long fTime = mdate.getTime() - (b + 7) * 24 * 3600000;
        for (int a = 1; a <= 7; a++) {
            fdate = new Date();
            fdate.setTime(fTime + (a * 24 * 3600000));
            list.add(a - 1, fdate);
